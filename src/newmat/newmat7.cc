...
static void AddDS(GeneralMatrix* gm, GeneralMatrix* gm1, GeneralMatrix* gm2)
{
   ...
   MatrixRow mr(gm, StoreOnExit+Node_pairPart);
...
// Add into first argument
{
   REPORT
   int nr = gm->Nrows();
   MatrixRow mr(gm, StoreOnExit+LoadOnEntry+Node_pairPart);
...
static void SubtractDS(GeneralMatrix* gm, GeneralMatrix* gm2)
{
   REPORT
   int nr = gm->Nrows();
   MatrixRow mr(gm, LoadOnEntry+StoreOnExit+Node_pairPart);
   MatrixRow mr2(gm2, LoadOnEntry);
   ...
   MultipliedMatrix* mm, MatrixType mtx)
{
   ...
   MatrixCol mcx(gmx, StoreOnExit+Node_pairPart);
   ...
   MultipliedMatrix* mm, MatrixType mtx)
{
   ...
   MatrixRow mrx(gmx, LoadOnEntry+StoreOnExit+Node_pairPart);
   ...
   BaseMatrix* sm, MatrixType mtx)
{
   ...
   Try
   {
      ...
      MatrixColX mcx(gmx, r, StoreOnExit+Node_pairPart);   // copy to and from r
...
GeneralMatrix* AddedMatrix::Evaluate(MatrixType mtd)
{
   ...
   GeneralMatrix* gmx = 0;				// pk initialize - warning from OS X gcc
...
GeneralMatrix* ConcatenatedMatrix::Evaluate(MatrixType mtx)
{
   ...
   Tracer tr("Concatenate");
      ...
      MatrixRow mr1(gm1, LoadOnEntry); MatrixRow mr2(gm2, LoadOnEntry);
      MatrixRow mr(gmx, StoreOnExit+Node_pairPart);
...
bool is_zero(const BaseMatrix& A)
{
   ...
   REPORT
   GeneralMatrix* gm1 = 0; bool bx = false;			// pk initialize bx - warning from OS X gcc
