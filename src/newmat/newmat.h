...
protected:
   int tag_value;                               // shows whether can reuse
...
public:
   ...
   void tDelete();                              // delete if tag_value permits
   bool reuse();                                // true if tag_value allows reuse
   void Protect() { tag_value=-1; }             // cannot delete or reuse
   int Tag() const { return tag_value; }
   bool is_zero() const;                        // test matrix has all zeros
   bool IsZero() const { return is_zero(); }    // test matrix has all zeros
   void Release() { tag_value=1; }              // del store after next use
   void Release(int t) { tag_value=t; }         // del store after t accesses
   void ReleaseAndDelete() { tag_value=0; }     // delete matrix after use
   void release() { tag_value=1; }              // del store after next use
   void release(int t) { tag_value=t; }         // del store after t accesses
   void release_and_delete() { tag_value=0; }   // delete matrix after use
...
public:
   ...
   void cleanup();                              // to clear store
   void MiniCleanUp()
      { store = 0; storage = 0; nrows_val = 1; ncols_val = 0; tag_value = -1; }
...
public:
   ...
   void cleanup();                              // to clear store
   void MiniCleanUp()
      { store = 0; storage = 0; nrows_val = 0; ncols_val = 1; tag_value = -1; }
//   ReturnMatrix Reverse() const;                // reverse order of elements
   void operator+=(const Matrix& M) { PlusEqual(M); }
   void operator-=(const Matrix& M) { MinusEqual(M); }
   void operator+=(Real f) { GeneralMatrix::Add(f); }
