...
// Assume cannot have both !Node_pairPart && StoreHere for MatrixRowCol routines
...
void SymmetricMatrix::GetRow(MatrixRowCol& mrc)
{
   ...
   if (+(mrc.cw*Node_pairPart))
      { REPORT mrc.storage=row+1; mrc.data=store+(row*(row+1))/2; }
   else
   {
      // do not allow StoreOnExit and !Node_pairPart
      if (+(mrc.cw*StoreOnExit))
...
void SymmetricMatrix::GetCol(MatrixRowCol& mrc)
{
   ...
   REPORT
   mrc.skip=0;
   if (+(mrc.cw*Node_pairPart))    // actually get row ??
...
// Do not need RestoreRow because we do not allow !Node_pairPart && StoreOnExit
