...
GeneralMatrix* GeneralMatrix::Transpose(TransposedMatrix* tm, MatrixType mt)
{
   ...
   if (Compare(Type().t(),mt))
   {
      ...
      {
         MatrixRow mr(gm1, StoreOnExit+Node_pairPart, i);
   ...
   else
   {
      ...
      MatrixCol mc(gm1, StoreOnExit+Node_pairPart);
      int i = nrows_val;
...
GeneralMatrix* GeneralMatrix::Evaluate(MatrixType mt)
{
   ...
   MatrixRow mr(this, LoadOnEntry);
   MatrixRow mrx(gmx, StoreOnExit+Node_pairPart);
   int i=nrows_val;
...
MatrixInput GetSubMatrix::operator<<(double f)
{
   ...
   MatrixRow mr(gm, Node_pairPart, row_skip);  // to pick up location and length
   int n = mr.Storage();
   if (n<=0)
   {
