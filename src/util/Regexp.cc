...
public:
	...
	const sstring & GetErrorString() const;
};
...
const sstring & CRegErrorHandler::GetErrorString() const
{
	return m_szError;
}
...
public:
	...
	sstring GetReplaceString( const char* sReplaceExp ) const;
...
#ifdef _DEBUG
	sstring m_originalPattern;
	sstring m_modifiedPattern;
#endif
};
...
sstring Regexp::GetReplaceString( char* source ) const
{
	ClearErrorString();
	if ( rc )
...
const sstring Regexp::operator[]( unsigned int i ) const
{
	ClearErrorString();
	assert( rc );
	if ( rc )
	{
		int len = SubLength(i);
		sstring buffer (rc->startp[i], len);
		return buffer;
	}
	else
	{
...
sstring regexp_internal::GetReplaceString( const char* sReplaceExp ) const
{
	sstring szEmpty;
	...
	sstring szReplace (replacelen, '\0');
...
sstring Regexp::GetErrorString() const
{
