...
my $xgram2hsm = "$perldir/xgram2hsm.pl";
...
foreach my $prefix (@prefix) {
    ...
    my $command = "$xrate $opts $stockFile -t $xrateFile";
    warn "[Comparing '$command' with '$xrangeFile']\n";
    system $command;

    local *XRATE;
    open XRATE, "$xgram2hsm $xrateFile|" or die "'$xgram2hsm $xrateFile|': $!";
	...
	for (my $i = 0; $i < @desiredOutput; ++$i) {
	    ...
	    my @d = split /\s+/, $d;
	    if (@x != @d) {
		warn "   [Line $i of xgram2hsm.pl output has ", @x+0, " fields; expected ", @d+0, "]\n";
		$testok = 0;
	    } else {
		for (my $j = 0; $j < @d; ++$j) {
		    my $dj = $d[$j];
