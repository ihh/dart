...
struct PDMCMC_main
{
  ...
  // constructors
  PDMCMC_main (int argc, char** argv);  // does not call init_opts, so caller can add grammars before initializing the Opts_List
  PDMCMC_main();  // calls init_opts implicitly, for embedded usage

  // destructor
  ~PDMCMC_main();

  // add a preset grammar
  // call this as many times as you like after constructing the PDMCMC_main object
  void add_grammar (const char* name, ECFG_scores* ecfg);
  void add_and_select_grammar (const char* name, ECFG_scores* ecfg);  // as well as adding the grammar, set it as the default "preset", thus forcing the PDMCMC_main object to use this grammar
