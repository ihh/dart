...
// PDMCMC_main
PDMCMC_main::PDMCMC_main (int argc, char** argv)
  : opts (argc, argv),
    max_subseq_len (0),
    alph (0),
...
void PDMCMC_main::add_grammar (const char* name, ECFG_scores* ecfg)
{
...
void PDMCMC_main::add_and_select_grammar (const char* name, ECFG_scores* ecfg)
{
...
void PDMCMC_main::add_standard_grammars (const char* default_grammar)
{
...
void PDMCMC_main::delete_grammars()
{
  for_contents (ECFG_map, ecfg_map, sg)
    delete (*sg).second;
}

void PDMCMC_main::init_opts (const char* desc)
{
...
void PDMCMC_main::read_alignments (const Stockholm& stock)
{
...
void PDMCMC_main::read_alignments (const Stockholm_database* stock)
{
...
void PDMCMC_main::estimate_trees (SExpr* grammar_alphabet_sexpr, Sequence_database* seq_db_ptr) {
  // HACK REPORT
...
void PDMCMC_main::copy_trees_to_stock_db()
{
...
void PDMCMC_main::read_grammars (SExpr* grammar_alphabet_sexpr)
{
...
void PDMCMC_main::convert_sequences()
{
...
void PDMCMC_main::train_grammars()
{
  // do training
  if (missing_trees())
...
void PDMCMC_main::delete_trainers()
{
...
void PDMCMC_main::delete_loaded_grammars()
{
  for_contents (vector<ECFG_scores*>, grammars_to_delete, g)
    delete *g;
}

void PDMCMC_main::annotate_alignments (ostream* align_stream)
{
  if (missing_trees())
...
bool PDMCMC_main::missing_trees() const
{
...
bool PDMCMC_main::unattached_rows() const
{
  return align_db.has_unattached_rows();
}

// run methods
void PDMCMC_main::run_xrate (ostream& alignment_output_stream)
{
  parse_opts();
  read_alignments();
  estimate_trees();
...
Stockholm& PDMCMC_main::run_tree_estimation (Stockholm& stock, Sequence_database& stock_seq_db, SExpr& grammar_alphabet_sexpr)
{
...
Stockholm& PDMCMC_main::run_alignment_annotation (Stockholm& stock, SExpr& grammar_alphabet_sexpr)
{
  read_alignments (stock);
...
void PDMCMC_main::run_grammar_training (Stockholm_database& stock, SExpr& grammar_alphabet_sexpr, ECFG_scores** grammar_ret, ECFG_counts** counts_ret)
{
  read_alignments (&stock);
  read_grammars (&grammar_alphabet_sexpr);
  convert_sequences();
...
ECFG_scores* PDMCMC_main::run_macro_expansion (SExpr& grammar_alphabet_sexpr) {
  read_grammars (&grammar_alphabet_sexpr);
  return grammar.size() > 0 ? grammar[0] : (ECFG_scores*) 0;
}

ECFG_scores* PDMCMC_main::run_macro_expansion (Stockholm& stock, SExpr& grammar_alphabet_sexpr) {
  read_alignments (stock);
  read_grammars (&grammar_alphabet_sexpr);
  return grammar.size() > 0 ? grammar[0] : (ECFG_scores*) 0;
}
