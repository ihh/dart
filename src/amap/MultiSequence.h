...
#include "amap/Sequence.h"
#include "amap/FileBuffer.h"
...
class MultiSequence {
 ...
 public:
  ...
  /// Convert a FASTA_sequence_database into a MultiSequence object.
  // RKB -- 2/9/08
  MultiSequence (const FASTA_sequence_database& seq_db)
    {

      // read in the info from the seq_db and store it like MultiSequence wants
      SafeVector<SafeVector<char> *> seqData;
      SafeVector<string> seqNames;
      SafeVector<int> seqLengths;
      for (int i = 0; i < (int) seq_db.size(); ++i)
	{
	  Named_profile np = *seq_db.index.profile[i];
	  seqData.push_back (new SafeVector<char>());
	  seqData[i]->push_back ('@');
	  for (int j = 0; j < (int) np.seq.size(); ++j)
	    seqData[i]->push_back ((char) np.seq[j]);
	  seqNames.push_back (np.name);
	  seqLengths.push_back ((int) np.size());
	}

      // now initialize a MultiSequence object
  ...
  /// Convert MultiSequence alignment to Stockade object.
  // Must be a Stockade object so that it stores its own Named_profile's.
  // RKB 3/8/08
  Stockade GetStockade() {

    Stockade stockade (GetNumSequences(), GetSequence (0)->GetLength());
    for (int r = 0; r < GetNumSequences(); ++r)
      {
	...
	Biosequence ungapped;
	for (int col = 0; col < stockade.align.columns(); ++col)
	  {
	    char ch = seq->GetPosition (col+1); // Sequence's have 1-based indexing but Stockholm's have 0-based indexing
	    if (stockade.align.is_gap_char (ch))
	      stockade.align.path.row (r)[col] = false;
	    else
	      {
		stockade.align.path.row (r)[col] = true;
