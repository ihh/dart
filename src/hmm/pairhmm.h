  ...
  for (int i = 0; i < states(); i++)
      ...
      if (state_type[i] == EmitX || state_type[i] == EmitY)
	{
	  ...
	  for (int x = 0; x < (int) single_emit[i].size(); x++)
	    { o.width(w); show_element(single_emit[i][x],o); if (x < (int) single_emit[i].size()-1) o << " "; }
	  o << ")\n";
	}
	  ...
	  for (int y = 0; y < pair_emit[i].ysize(); y++)
	    {
	      if (y > 0) o << "                  ";
	      o << "(";
	      for (int x = 0; x < pair_emit[i].xsize(); x++) { o.width(w); show_element(pair_emit[i](x,y),o); if (x < pair_emit[i].xsize()-1) o << " "; }
	      o << ")";
